...
rents a VM. GitHub CLI reads and writes plus the recipe push get three bounded attempts so a transient transport error
does not fail discovery or strand a pushed recipe commit. Discovery uses a bounded research prompt and a
workflow-specific model-turn cap so the manifest is written before the agent transcript reaches the inference
