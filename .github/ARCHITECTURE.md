...
compares qualified targets and rejects an obsolete decision when the replacement's smallest deployment needs more
total physical GPU memory than the old recipe's smallest deployment; the agent must use `best-effort` when the old
model retains any material capability or operating advantage. Obsolete recipes remain in git but cannot be deployed,
benchmarked, published, or bundled; a later reassessment may return one to the maintained or best-effort set. The
workflow also creates
