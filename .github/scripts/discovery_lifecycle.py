    ...
    *,
    ...
    for decision in value:
        ...
        decisions.append(
            {
                "model_id": model_id,
                "rationale": _rationale(decision["rationale"], model_id),
                "heat": validate_model_heat(decision["heat"], model_id, required=True),
            }
        )
...
def _obsolete_decisions(value: object, records: dict[str, dict], *, ignore_invalid_ids: bool = False) -> list[dict]:
    if not isinstance(value, list):
...
def _model_ids(decisions: list[dict]) -> list[str]:
    return [decision["model_id"] for decision in decisions]


def _retain_best_effort(decision: dict, records: dict[str, dict]) -> dict:
...
def validate_manifest(path: Path, workspace: Path) -> dict:
    ...
    existing_onboarding = {model_id for model_id, record in records.items() if ONBOARDING_TAG in record["tags"]}
    ...
    for candidate in candidates:
        ...
        if model_id in existing_onboarding:
            config = records[model_id]["config"]
        ...
        normalized_candidates.append(
            {
                ...
                "heat": validate_model_heat(candidate["heat"], model_id, required=True),
                "deployments": normalized_deployments,
            }
        )

    missing_onboarding = existing_onboarding - candidate_ids
    if missing_onboarding:
        raise ValueError(f"Existing onboarding models must be scored: {', '.join(sorted(missing_onboarding))}")
    ...
    if unclassified:
        raise ValueError(f"Complete recipes must be scored and classified: {', '.join(sorted(unclassified))}")
...
def _replace_model_metadata(text: str, rationale: str, heat: int) -> str:
    lines = text.splitlines(keepends=True)
    ...
    while model_end < len(lines) and (not lines[model_end].strip() or lines[model_end].startswith((" ", "\t"))):
        model_end += 1
    field_starts = [index for index in range(model_start + 1, model_end) if lines[index].startswith(("  rationale:", "  heat:"))]
    for start in reversed(field_starts):
        end = start + 1
        while end < model_end and not (lines[end].startswith("  ") and not lines[end].startswith("    ") and ":" in lines[end]):
            end += 1
        del lines[start:end]
        model_end -= end - start
    ...
    lines[huggingface + 1 : huggingface + 1] = [
...
def _model_lines(decisions: list[dict]) -> list[str]:
    return [f"- **{decision['heat']}** · `{decision['model_id']}` — {decision['rationale']}" for decision in decisions] or ["- None."]


def _obsolete_lines(decisions: list[dict]) -> list[str]:
    if not decisions:
        return ["- None."]
    lines = []
    for decision in decisions:
        ...
        lines.append(f"- **{decision['heat']}** · `{decision['model_id']}`{successor} — {decision['rationale']}")
    return lines
...
def _onboarding_lines(decisions: list[dict]) -> list[str]:
    ...
    for decision in decisions:
        ...
        lines.append(f"- **{decision['heat']}** · `{decision['model_id']}` on {deployments} — {decision['rationale']}")
    return lines
...
def _summary(manifest: dict) -> str:
    ...
    if manifest["onboarding_models"]:
        lines.extend(_onboarding_lines(sorted(manifest["onboarding_models"], key=lambda decision: -decision["heat"])))
    else:
        lines.append("- None.")
...
def apply_manifest(manifest: dict, workspace: Path, summary_path: Path) -> dict:
    ...
    for decision in manifest["best_effort_models"]:
        changed = _set_lifecycle(records[decision["model_id"]], BEST_EFFORT_TAG, decision["rationale"], decision["heat"]) or changed
