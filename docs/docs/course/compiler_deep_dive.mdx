...
**Tracing** observes a program with example inputs and records the operations it performs. It happens before
**decomposition**, which breaks compound operations into simpler math. The tracer's job is to preserve the operation PyTorch expressed, not to
