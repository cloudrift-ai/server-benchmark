...
│   ├── source: Reduction
│   │   ├── axis: a2
│   │   ├── carrier: Carrier(...)  # state acc0, combined with add
│   │   ├── partial: Body[Load, Assign, Accum]
│   │   ├── role: PLANAR
│   │   └── reduce: ReducePlan(...)
│   └── body: Body[Load constants, Assign(v0), Assign(v2), Assign(v3), Assign(v4), Loop(a3, ...)]
├── place: Placement(...)
├── tier: TilePlan(...) or None
├── stage: Stage(...) or None
└── workers: WarpSpec(...) or None
```

The dataflow graph is still small, but the node now contains a scheduled structural tree. For RMSNorm, the root is a
`Map` whose `source` is a `Reduction`; the post-reduction projection remains in the map body. Schedule descriptors are
fields on `TileOp` or its structural nodes rather than separate graph nodes.
...
or matrix multiplication. Loop IR says what iterations and arithmetic occur, but not what high-level algebra they
form. `lowering/tile` analyzes the statement tree and creates structural nodes:
...
- **Map** applies a function independently at each output position: `y[i] = f(x[i])`. The function is *mapped over*
  the elements, so one position does not need a result from another. Emmy also uses `Map` as a projection wrapper
  around another structural node, such as the normalization performed after a sum.
- **Reduction** combines many values into fewer values with an associative operation: `y = sum_i x[i]`. It *reduces*
  the collection, and the reduced axis disappears from the output. Sum, maximum, and minimum are familiar examples.
- **Contraction** is the tensor-algebra name for pairing indexed operands and reducing a shared index. In
  `C[m,n] = sum_k A[m,k] * B[k,n]`, `k` occurs in both operands but not in the result: it has been *contracted away*.
  A contraction is a specialized reduction, but Emmy represents it separately because its reuse and GPU schedules
...
| Loop IR shape | Tile IR structure | Meaning |
|---|---|---|
| Free loops containing independent scalar work | `Map(body=..., source=None)` | Calculate each output cell independently |
| One associative fold | `Reduction(...)` | Combine values along one axis |
| A fold followed by post-processing | `Map(source=Reduction(...), body=projection)` | Reduce first, then project the result |
| A product-like lift followed by an additive fold | `Contraction(..., epilogue=...)` | Contract indexed operands and carry its bound epilogue |
| A larger projection around structural work | `Map(source=Reduction(...) or Contraction(...), body=...)` | Compose more work around a structural source |
| Structure that cannot be classified safely | Flat `Map(body=original_statements)` | Preserve Loop IR semantics and use the scalar tier |

A `Map` is therefore more than the pointwise case. It is also the projection wrapper that composes with a reduction or
contraction:

```text
pure pointwise:         Map(body=pointwise, source=None)
projected reduction:    Map(body=projection, source=Reduction(...))
projected contraction:  Map(body=epilogue, source=Contraction(...))
```

A `Reduction` stores the parts that make an associative fold explicit:

```text
Reduction
├── axis: the dimension being combined
├── carrier: accumulated state, identity, and combine operation
├── partial: work that produces one contribution
├── role: PLANAR or TWISTED
└── reduce: ReducePlan describing its selected partition
```

A `Contraction` is a specialized fold. Its two output axes and one reduction axis are bound once:
...
├── k_axis: k
├── a_operand: A[m, k]
├── folds: B[k, n] and the accumulator they update
├── tile: TilePlan
└── epilogue: work after the fold
```
...
The small vocabulary does not mean the recognizer must force every loop nest into a high-level category. A flat
`Map` can retain unclassified `Loop`, `Load`, `Assign`, `Accum`, and `Write` objects unchanged. This conservative
escape hatch preserves correctness when the compiler cannot prove a stronger algebraic interpretation.
...
4096.” It recognizes that the lift distributes over the fold and that at least two bindable reduction-axis operand
loads participate.
...
is recognized as a contraction because multiply distributes over add and the body contains two bindable
...
The boundary is implemented by `pipeline/passes/lowering/tile/010_recognize.py`. It matches a `LoopOp`, reads its
statement tree, creates one of the structural forms above, wraps it in a `TileOp`, and invokes scheduling. Recognition
does not use variable names such as `a2` or `acc0` to guess meaning. It examines statement types, dependency edges,
index expressions, loop roles, and algebraic properties of the elementwise operations.

```mermaid
flowchart TD
    Loop["LoopOp.body"] --> Special{"Special streaming form?"}
    Special -->|Flash attention| Flash["Build fused flash TileOp"]
    Special -->|Online softmax| Softmax["Annotate TWISTED carrier"]
    Special -->|Neither| Peel["_peel: separate outer free axes<br/>from the per-cell body"]
    Softmax --> Peel
    Peel --> Count{"One flat<br/>reduction loop?"}

    Count -->|No or ambiguous| Flat["Flat Map fallback<br/>retain original statements"]
    Count -->|Yes| Annotate["_annotate_reduce:<br/>recover role and carrier"]
    Annotate --> Role{"Recovered role"}

    Role -->|PLANAR or TWISTED| Reduction["Reduction"]
    Role -->|CONTRACTION| Bind{"Bind operands and<br/>output axes m, n, k?"}
    Role -->|No safe carrier| Flat

    Bind -->|Yes| Contraction["Contraction"]
    Bind -->|No| Demote["Demote to PLANAR Reduction"]

    Reduction --> Projection{"Projection remains?"}
    Contraction --> ContractNode["Contraction<br/>carries its bound epilogue"]
    Demote --> Projection
    Projection -->|Yes| Wrap["Map(source=node,<br/>body=projection)"]
    Projection -->|No| Root["Structural node is root"]

    Flat --> Tile["TileOp + Placement"]
    Wrap --> Tile
    Root --> Tile
    ContractNode --> Tile
    Flash --> Schedule
    Tile --> Schedule["Enumerate and select<br/>legal schedules"]
```

The implementation proceeds in the following order.

#### 1. Recognize streaming special cases first

Flash attention and online softmax must be recognized before the general lift because they combine several ordinary
folds into one streaming state:

- `try_flash` looks for the compatible score, softmax, and value-contraction structure and can rewrite more than one
  graph node into a fused flash `TileOp`.
- `_softmax._fuse` recognizes the adjacent row-maximum and sum-of-exponentials folds and gives the resulting loop a
  multi-component `TWISTED` carrier.

These are structural recognizers, but they are currently dedicated cases. Emmy does not yet have one universal
algorithm that derives every possible algebra from arbitrary loops.

#### 2. Peel off independent axes

`_peel` walks through the outer chain of non-reduction loops:

```text
for batch:                 free
    for row:               free
        for k:             reduction begins
            acc += ...
```
...
```text
free axes = [batch, row]
per-cell body = [the k reduction and its projection]
```

The free axes belong to placement and later map onto the CUDA grid. Leading pure statements such as invariant loads or
assignments are moved into the per-cell body so their dependencies remain available.

#### 3. Find a classifiable fold

`_lift_cell` counts top-level reduction loops in the per-cell body:

- no reduction means a pure pointwise `Map`;
- one flat reduction is eligible for annotation; and
- several reductions or an unsupported nested reduction remain a flat `Map`.

For one reduction, dependency analysis separates statements before the loop into those feeding the fold and those
feeding only the projection. Fold dependencies move into the reduction body; projection-only dependencies remain
after it. If one statement must live on both sides and cannot be moved safely, recognition declines to the flat-map
fallback rather than changing evaluation order.

#### 4. Recover the reduction role and carrier

`_annotate_reduce` inspects the chosen loop:

1. A loop already annotated by online-softmax or flash recognition keeps its carrier and role.
2. A clean lift-and-fold pattern receives the `CONTRACTION` role.
3. A loop containing exactly one `Accum` becomes `PLANAR`; `Accum.as_carrier()` supplies its state, identity, and
   combine operation.
4. A loop with no unambiguous carrier remains inside a flat `Map`.
...
the fold operation supplies an additive carrier with zero as its identity. Emmy can later partition the axis because
addition is associative: separate workers may form partial sums and combine them without changing the intended result.

#### 5. Prove contraction structure and bind operands

`_is_clean_contraction` requires more than the presence of multiplication:

- exactly one `Accum`;
- an `Assign` whose result is the value folded by that accumulator;
- at least two operand loads indexed by the reduction axis;
- a lift operation that distributes over the fold operation; and
- a body whose operand preprocessing can be assigned safely to the contraction.

The operand loads may be different affine views of one packed buffer, as with packed QKV. Their load expressions and
indices identify the operands; backing-buffer identity alone does not. Conversely, `sum(x * x)` does not become a
matrix contraction merely because `x` appears twice.

After the free axes are ordered by the output layout, `_nodify_contraction` tries to bind the final two as `(m, n)`,
the reduce axis as `k`, and the indexed loads as the A and B operands. If the shape or indexing cannot support that
binding—for example, a one-dimensional output with no usable `(m, n)` pair—the loop is demoted to a `PLANAR`
`Reduction`. It still compiles through the general reduction machinery.

#### 6. Separate the fold from its projection

A bare reduction becomes a root `Reduction`. If work remains after that fold, recognition stores it in a wrapping
`Map`:

```text
Map
├── source: Reduction
└── body: post-fold projection
```

For RMSNorm, the `a2` sum is the `Reduction`; division, epsilon, reciprocal square root, the `a3` sweep, and the final
write form the projection map.

A `Contraction` carries the epilogue bound directly to its output cells in `Contraction.epilogue`. In larger composed
forms, such as a fused statistic feeding a contraction, a `Map` may additionally wrap the `Contraction` as its
structural source. This distinction keeps ordinary contraction epilogues near their operand binding while still
allowing higher-level composition.

#### 7. Attach placement and schedule choices

Recognition constructs an initially unmapped wrapper:
...
The scheduling half then maps free axes to the grid and offers the legal `ReducePlan`, `TilePlan`, staging, and worker
alternatives. Recognition determines which transformations preserve the algebra; scheduling chooses among their
hardware realizations.

One useful invariant closes the loop: `tile.ops.lower` can flatten `Map`, `Reduction`, and `Contraction` back into
equivalent loop statements. Tile recognition adds algebraic meaning and schedule metadata; it does not change the
program's mathematical result.
