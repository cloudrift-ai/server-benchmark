...
Read lessons 6–12 in [Learn Emmy: A Code-First Course](./project_course) first. Keep the
[Emmy Course Glossary](./glossary) open for quick definitions. Every stage below preserves the intended answer; it only
changes how explicitly the computation is described.
:::
...
:::info How to read the in-memory views
Each stage below now has two views. The first is Emmy's ordinary printed listing. The second is a schematic of the
Python objects in memory:

- arrows are dataflow edges in the persistent `Graph`;
- `Node(op=...)` is a graph node containing an operation object from the current IR dialect;
- indented objects inside `LoopOp`, `TileOp`, and `KernelOp` are nested structure, not additional graph nodes; and
- labels use the real class names and important fields, while omitted details and `...` keep the diagram readable.

These schematics are not Python source to paste or execute. Emmy creates the objects during tracing and rewriting, then
`Graph.pretty_print()` and each operation's `pretty_body()` produce the human-readable listings.
:::

The outer representation changes like this:

```mermaid
flowchart LR
    Frontend["Frontend IR<br/>3 graph nodes<br/>compute op = RmsNormOp"]
    Tensor["Tensor IR<br/>15 graph nodes<br/>11 primitive compute ops"]
    Loop["Loop IR<br/>5 graph nodes<br/>compute op = LoopOp"]
    Tile["Tile IR<br/>same outer graph<br/>compute op = TileOp"]
    Kernel["Kernel IR<br/>same outer graph<br/>compute op = KernelOp"]
    CUDA["CUDA IR<br/>same outer graph<br/>compute op = CudaOp"]

    Frontend -->|decompose| Tensor
    Tensor -->|lift and fuse| Loop
    Loop -->|rebind op| Tile
    Tile -->|rebind op| Kernel
    Kernel -->|render source| CUDA
```
...
**In-memory graph (schematic):**

```mermaid
flowchart LR
    subgraph Graph["Graph — 3 nodes"]
        X["x<br/>Node(op=InputOp)"]
        Weight["p_weight<br/>Node(op=ConstantOp)"]
        RMS["rms_norm<br/>Node(op=RmsNormOp)"]
...
The compute node contains:

```text
Node(
    inputs=[x, p_weight],
    op=RmsNormOp(eps=1e-06),
    outputs=(Tensor(shape=(2, 8, 16), dtype=f32),),
)
```
...
```mermaid
flowchart LR
    subgraph Graph["Graph — 15 nodes"]
        X["x<br/>InputOp"]
        Weight["p_weight<br/>ConstantOp"]
        Eps["rms_norm_eps<br/>ConstantOp"]
        Count["rms_norm_mean_count<br/>ConstantOp"]

        WeightBC["p_weight_bc<br/>IndexMapOp"]
        Square["rms_norm_sq<br/>ElementwiseOp: multiply"]
        EpsBC["rms_norm_eps_bc<br/>IndexMapOp"]
        CountBC["rms_norm_mean_count_bc<br/>IndexMapOp"]
        Sum["rms_norm_mean_sum<br/>ReduceOp: sum"]
        Mean["rms_norm_mean<br/>ElementwiseOp: divide"]
        AddEps["rms_norm_add_eps<br/>ElementwiseOp: add"]
        RSqrt["rms_norm_rsq<br/>ElementwiseOp: rsqrt"]
        RSqrtBC["rms_norm_rsq_bc<br/>IndexMapOp"]
        Norm["rms_norm_norm<br/>ElementwiseOp: multiply"]
        Output["rms_norm<br/>ElementwiseOp: multiply"]

        Weight --> WeightBC --> Output
        X --> Square --> Sum --> Mean --> AddEps --> RSqrt --> RSqrtBC --> Norm --> Output
        X --> Norm
        Eps --> EpsBC --> AddEps
        Count --> CountBC --> Mean
    end
...
Every named box is a separate `Node` in the same `Graph`. Each node's `op` is a tensor-dialect object; tensor shapes
remain on the node's output `Tensor`. The diagram includes the broadcast `IndexMapOp` nodes that the listing refers to
through names such as `rms_norm_rsq_bc`.
...
**Outer in-memory graph (schematic):**

```mermaid
flowchart LR
    subgraph Graph["Graph — 5 nodes after fusion"]
        X["x<br/>InputOp"]
        Count["rms_norm_mean_count<br/>ConstantOp"]
        Eps["rms_norm_eps<br/>ConstantOp"]
        Weight["p_weight<br/>ConstantOp"]
        RMS["rms_norm<br/>Node(op=LoopOp)"]
...
**Nested operation payload:**

```text
LoopOp
└── body: Body
    ├── Load(rms_norm_mean_count)
    ├── Assign(v0)
    ├── Load(rms_norm_eps)
    └── Loop(axis=a0)
        └── body: Body
            └── Loop(axis=a1)
                └── body: Body
                    ├── Loop(axis=a2)
                    │   └── body: Body[Load, Assign, Accum]
                    ├── Assign(v2)
                    ├── Assign(v3)
                    ├── Assign(v4)
                    └── Loop(axis=a3)
                        └── body: Body[Load, Assign, Load, Assign, Write]
```

Only the input and constant boundary nodes plus the fused `rms_norm` compute node remain. `Loop`, `Load`, `Assign`,
`Accum`, and `Write` are statement objects inside the single `LoopOp.body` tree. Fusion removed the intermediate tensor
nodes and moved their work into that body. The printed listing inlines scalar constant loads as `16` and `1e-06`; the
underlying graph and body retain those constant inputs so later code generation can supply them.
...
### Stage 4 — Tile IR lifts algebra and attaches a schedule
...
=== 0: k_rms_norm_<hash> ===
    ...
    work   t128
    Fold  free
    ├─ operand[acc0]: Fold[a1 in 0..16] planar   <REDUCE=coop>   <computed>
    │  ├─ init: (0)
    │  ├─ lift: lambda(a1) -> (v1)
    │  │    in2 = load x[(a0 // 8), (a0 % 8), a1]
    │  │    v1 = multiply(in2, in2)
    │  └─ combine: lambda(acc0, acc0__o) -> (acc0)
    │       acc0 = add(acc0, acc0__o)
    └─ lift: lambda(acc0) -> (v6)
         ... reciprocal, epsilon, rsqrt, scale, and weight ...
    outputs
    └─ sweep(a2) rms_norm[(a0 // 8), (a0 % 8), a2] = v6
```

**In-memory structural operation tree (schematic):**

The outer `Graph` has the same five nodes and four input edges shown in Stage 3. Lowering rebinds the compute node's
operation from `LoopOp` to `TileOp`; the nested payload becomes:

```text
TileOp
├── op: Fold(axis=None)                 # zero-axis projection
│   ├── operands: Fold(axis=a1)         # planar sum
│   │   ├── lift: Lambda(...)
│   │   ├── init: (0)
│   │   └── combine: Lambda(add)
│   └── lift: Lambda(...)               # normalize and scale
├── place: Placement(free=(a0), grid=(a0))
├── output_specs: sweep(a2) -> rms_norm
└── schedule: Schedule(...)             # choices indexed by Fold-tree sites
```

The dataflow graph is still small, but the node now contains one scheduled pure `Fold` tree. A zero-axis `Fold` is the
map/projection reading; an iterating `Fold` stores a reduction's lift, initial state, and associative combine. Roles
such as planar, twisted, and contraction are derived from that one representation rather than stored as different
node classes. The accepted `Schedule` is indexed by sites in the immutable tree.

The important new fact is `Fold[a1] planar`. Total lifting made the sum's algebra explicit: its partial states combine
with addition. Tile IR keeps that pure mathematical structure separate from the selected schedule so alternative valid
realizations can be compared without rebuilding the term.
...
**In-memory hardware statement tree (schematic):**

The outer dataflow is unchanged again. The `rms_norm` node now contains `KernelOp`:

```text
KernelOp
├── name: k_rms_norm_<hash>
└── body: Body
    └── Tile(axes=[a0, a1, a2_co])
        └── body: Body
            ├── StridedLoop(axis=a2, start=a2_co, step=16)
            │   └── body: Body[Load, Assign, Accum]
            ├── WarpShuffle(acc0, length=16)
            ├── Load(rms_norm_mean_count)
            ├── Load(rms_norm_eps)
            ...
            ├── Assign(v3, dtype=f32)
            ├── Assign(v4, dtype=f32)
            └── StridedLoop(axis=a3, start=a2_co, step=16)
                └── body: Body[Load, Load, Assign, Write]
```

The `TileOp` algebra tree has been materialized into a `KernelOp.body` tree. Hardware actions such as cooperative worker
coordinates and `WarpShuffle` are statement objects inside that body. They are not Python calls executed to perform
the normalization; the CUDA lowering pass consumes them as data.
...
**In-memory rendered-source object (schematic):**

The outer graph still has the Stage 3 shape. Only the compute node's operation has been rebound:

```text
CudaOp
├── kernel_source: "extern \"C\" __global__ ... }"
├── kernel_name: "k_rms_norm_<hash>"
├── arg_order: ("x", "rms_norm_mean_count", "rms_norm_eps", "p_weight", "rms_norm")
├── grid: (...)
├── block: (...)
├── smem_bytes: 0
├── zero_outputs: (...)
...
This is the one stage where the operation object contains real generated source text: `CudaOp.kernel_source`.
The surrounding `Graph` still expresses kernel launch order and buffer dependencies, while the remaining `CudaOp`
fields tell the runtime how to supply arguments and launch that source.
...
that each part's fingerprint incorporates the fingerprints of the parts it depends on. The key includes operation
structure, tensor shape and dtype, input relationships, and graph inputs/outputs. It excludes cosmetic node and tensor
names, hints, and mutable internal ids.
...
## 8. Total lift: express Loop IR as one pure Fold tree

**Algebra** here means the mathematical structure of the work—for example, whether a loop is a sum, a pointwise map,
or matrix multiplication. Loop IR already states each reduction through `Accum` operations. Tile lowering mechanically
lifts every reduction into a `Fold` and stores the complete result as one canonical pure tree. This **total lift** has
no raw-loop fallback or round-trip recognition gate.

`Fold` is the only stored structural node. It contains an optional axis, a pure lift lambda, monoid initialization and
combine programs, and operand edges. Familiar forms are derived readings of those fields:

- a zero-axis `Fold` is pointwise work or a projection;
- an iterating planar `Fold` is an ordinary associative reduction;
- a twisted `Fold` carries stable multi-component state for online softmax or attention; and
- a contraction is a bilinear lift followed by an additive fold.

### Why these names?

The names come from functional programming and tensor algebra. They describe how values relate mathematically, not
which GPU instruction will execute them:

- **Map** applies a function independently at each output position: `y[i] = f(x[i])`. In stored Tile IR this is the
  zero-axis reading of `Fold`, often wrapping an operand `Fold` such as the normalization performed after a sum.
- **Reduction** combines many values into fewer values with an associative operation: `y = sum_i x[i]`. In stored
  Tile IR this is an iterating `Fold`; the reduced axis disappears from the output.
- **Contraction** is the tensor-algebra reading for pairing indexed operands and reducing a shared index. In
  `C[m,n] = sum_k A[m,k] * B[k,n]`, `k` occurs in both operands but not in the result: it has been *contracted away*.
  A contraction is a specialized reduction. Emmy derives a distinct contraction view because its reuse and GPU
  schedules are substantially different, while storing it in the same Fold representation.

The three basic shapes are therefore:

```text
Map:         y[i]   = f(x[i])
Reduction:   y      = sum_i x[i]
Contraction: C[m,n] = sum_k A[m,k] * B[k,n]
```

Here are the smallest useful examples. Start with one vector:
...
Every position produces its own result independently. The input has three elements and the output has three elements.
...
Three input values are combined into one output value, so the vector's only axis is reduced away.

**Contraction—take the dot product of two vectors:**
...
This is the smallest contraction: pair values that share the index, multiply each pair, and reduce the products by
addition. The shared vector index disappears from the scalar result. Matrix multiplication performs a similar dot
product for every output row-and-column pair.

These algebraic forms correspond to the following Loop IR patterns:

| Loop IR shape | Fold-tree reading | Meaning |
|---|---|---|
| Free loops containing independent scalar work | zero-axis `Fold` | Calculate each output cell independently |
| One associative reduction loop | iterating planar `Fold` | Combine values along one axis |
| A fold followed by post-processing | zero-axis `Fold` with a `Fold` operand | Reduce first, then project the result |
| A product-like lift followed by an additive fold | contraction reading of `Fold` | Contract indexed operands |
| Stable exponential state | twisted reading of `Fold` | Combine pivot and expectation state safely |

A zero-axis `Fold` is therefore more than the pointwise case. It is also the projection wrapper that composes with an
iterating fold:

```text
pure pointwise:         Fold(axis=None, lift=pointwise)
projected reduction:    Fold(axis=None, operands=(Fold(axis=k, ...),), lift=projection)
projected contraction:  Fold(axis=None, operands=(Fold.contraction(...),), lift=epilogue)
```

A `Fold` stores the parts that make an associative reduction explicit:

```text
Fold
├── axis: the dimension being combined, or None for a projection
├── operands: closed input edges, which may themselves be Folds
├── lift: pure work that produces one contribution
├── init: the monoid identity state
└── combine: the associative state × state -> state program
```

A contraction is a derived reading of one `Fold`. Its bilinear operands and reduction channels are recovered from the
same stored fields:
...
├── axis: k
├── a: A[m, k]
├── channels: B[k, n] paired with accumulator state
├── init/combine: additive monoid
└── schedule site: selected separately on TileOp
```

“Contraction” means a tensor contraction, not the act of merging graph nodes. Ordinary matrix multiplication is the
familiar example:
...
Total lift does not force every fold to be a contraction or twisted carrier. An unclassified reduction remains a
plain planar `Fold` and uses the scalar scheduling tiers. What it may not do is retain a raw reduction loop inside Tile
IR: every `Accum` loop has one structural representation, and output effects become explicit output specifications.

The RMSNorm loop becomes a planar fold used by a zero-axis projection:
...
The operand fold owns the sum. The zero-axis parent owns the work performed after it. This corresponds to:
...
Classification derives an `AxisRole` from the stored fold algebra:
...
4096.” It derives contraction when the lift is bilinear over the additive fold and its operand channels are bindable.
...
is classified as a contraction because multiply distributes over add and the body contains two bindable
`k`-indexed operand loads. Those loads can be different affine views of one packed buffer; a squared reduction
`sum(x * x)` is not classified as a contraction merely because the expression mentions `x` twice.

The contraction view binds its M, N, and K roles once. Later passes consume those derived roles instead of repeatedly
guessing operand meaning from indices.

### How Loop IR becomes Tile IR

The boundary begins in `pipeline/passes/lowering/tile/010_lift.py`, which delegates to `_fromloop.lift_loop_op`. It
matches a `LoopOp`, peels its outer free-axis chain into `Placement`, mechanically replaces every reduction loop with a
`Fold`, extracts output effects as `OutputSpec` values, and emits an unmapped `TileOp`. Later passes rewrite twisted
clusters, enumerate structural cuts, and schedule the immutable Fold tree.

```mermaid
flowchart TD
    Loop["LoopOp.body"] --> Peel["Peel outer free axes<br/>into Placement"]
    Peel --> Lift["Lift every reduce Loop<br/>into Fold"]
    Lift --> Outputs["Extract writes<br/>into OutputSpecs"]
    Outputs --> Check{"Any raw inner Loop<br/>or unrepresented effect?"}
    Check -->|yes| Error["Reject the invalid boundary"]
    Check -->|no| Tile["Unmapped TileOp<br/>with one Fold tree"]
    Tile --> Twist["Rewrite exp-family<br/>twisted clusters"]
    Twist --> Cuts["Enumerate placement and<br/>cross-CTA cuts"]
    Cuts --> Schedule["Enumerate and select<br/>site-indexed schedules"]
```
...
`_fromloop._peel` walks the outer chain of non-reduction loops. Those axes become `Placement.free`; leading pure
statements remain in the cell so their dependencies are preserved.

```text
for batch:                 free placement axis
    for row:               free placement axis
        for k:             reduction Fold axis
            acc += ...
```

#### 2. Lift every reduction mechanically

`lift_body` recursively replaces each reduction `Loop` with a `Fold`. Its `Accum` operations determine the state names
and componentwise monoid, while the remaining statements become the lift lambda. A scan—a fold whose per-step writes
observe its carried state—adds an observer lambda and returns those writes for output extraction.

For an ordinary sum, zero is the initial state and addition is the stored state-combine program. Associativity is what
allows later schedules to partition and recombine the axis.

#### 3. Extract the program boundary

`extract_output_specs` removes final writes from the pure term and records their destination indices and optional sweep
axes as `OutputSpec` values. `lift_loop_op` rejects any remaining raw inner loop or output effect it cannot represent.
That fail-loud check is what makes the Loop-to-Tile boundary total.

#### 4. Build one immutable Fold tree

The cell becomes a zero-axis `Fold.projection`, with nested folds connected as operands. A contraction is stored through
the same `Fold` fields and discovered by algebraic accessors; a squared reduction such as `sum(x * x)` remains planar
because it does not expose the required distinct bilinear operand channels.

```text
TileOp(
    op=Fold.projection(body=cell, operands=(nested_folds, ...)),
    place=Placement(free=free_axes),
    output_specs=outputs,
)
```

#### 5. Rewrite twisted clusters

`020_twisted.py` applies the general exponential-family rewrite to compatible Fold clusters before scheduling. Stable
online softmax and flash-attention state derive their multi-component streaming combine from the stored monoid rather
than carrying a separate hand-coded reduction node.

#### 6. Enumerate structural cuts

`030_cut.py` runs to a fixed point. It first offers cuts along closed stored-Fold edges, then cross-CTA reduction
splits. A cut may return a graph fragment with fresh kernels, so every fresh unscheduled `TileOp` re-enters the same
lifted-tree scheduling path.

#### 7. Assign a schedule

`040_schedule.py` projects the immutable Fold tree into classic scheduling domains, restricts them by pins and the
target, and enumerates `Schedule` assignments. The generic scheduler walks site-indexed kernel, node, and edge choices;
the classic adapter supplies the concrete `WORK`, `TILE`, `REDUCE`, `STAGE`, and `RASTER` vocabulary.

`Fold.lower()` is the single boundary that synthesizes equivalent Loop statements from the pure term. Schedule choices
live on `TileOp`, so changing a schedule never mutates the algebra or its structural identity.
...
and schedule separate. One `Schedule` assigns choices to paths in the Fold tree:

- `Placement` maps independent axes to the CUDA grid, the full collection of thread blocks for one launch.
- `Reduce` chooses grid-split, cooperative-block, or register-local reduction behavior.
- `Tile` describes contraction tiles, units, register cells, and hardware atom.
- `Stage` describes movement from large GPU global memory into faster per-block shared memory and its pipeline depth.
- `Work` describes thread or warp worker geometry.
- `Raster` describes traversal order when the schedule uses one.
...
the scheduled Fold tree. Its first major rule, `010_materialize`, follows the generic Fold-tree walk, reads the
schedule assignment for each site, and emits a `KernelOp` body.
...
For a wider reduction, a `Reduce` choice can instead add a cooperative lane axis. Each lane folds a strided portion, then
...
The preferred compiler path invokes offline nvcc to produce a **cubin**, a compiled CUDA binary, caches it by source,
target, and flags, then loads the symbol through cupy. If nvcc is absent or compilation fails, the runtime can fall
back to cupy's NVRTC path.

`EMMY_NVCC_FLAGS` participates in both cubin and performance-context keys. Tune and deployment default to the same
deployable `-O3` regime. The correctness test lane uses `-Xcicc -O1` only to reduce compilation time; it does not
measure performance, and a deliberately non-default tune remains isolated by its context key.
...
3. Tries an exact verified golden for the live GPU and pin regime.
4. Tries compatible deployable measurements in the online reservoir, then the tune DB.
5. Otherwise asks the trustworthy online prior, or its fitted offline cold-start fallback.
6. Uses the first emitted option only when every evidence and prior tier is silent.

If a chosen tile later fails validation or leaves an operation unlowered, the pipeline blocklists it and resolves
again. Structural choices can be retired as a group. A final option-zero resolution provides a bounded validity
fallback, but enumeration order makes no performance promise. If no valid lowering exists, compilation fails loudly.
...
This section gives the compiler-facing boundary. The full data model, score units, evidence order, CatBoost
persistence, strict deploy-identity join, A/B gates, and evaluation workflow are in
...
| Loop -> tile | Were all reductions lifted into the correct Fold tree and were output specifications preserved? |
...
4. Inspect Tile IR to check total lift, Fold classification, structural cuts, and schedule selection.
...
7. `pipeline/passes/lowering/tile/010_lift.py` and `_fromloop.py`: total Fold lifting and output extraction.
8. `compiler/ir/pure/fold.py`, `tree.py`, and `compiler/ir/tile/ir.py`: pure terms and the Tile boundary.
9. `pipeline/passes/lowering/tile/020_twisted.py`, `030_cut.py`, and `040_schedule.py`.
10. `pipeline/passes/lowering/kernel/010_materialize.py` and `_factor.py`.
11. `compiler/ir/kernel/render.py`.
12. `pipeline/passes/lowering/cuda/010_lower_kernelop.py`.
13. `backend/cuda/program.py`: compilation, allocation, and launch.
...
5. `compiler/ir/pure/fold.py`: the derived contraction view and operand channels.
6. `compiler/ir/schedule/` and `040_schedule.py`: generic traversal and the classic schedule codec.
...
4. `020_twisted.py` and `_twist.py`: exponential-family Fold rewriting and structural composition.
...
Find the rule that changes the kernel set, the Fold accessor that classifies contraction algebra, and the rule that
...
./venv/bin/pytest \
  ...
  tests/compiler/passes/test_total_lift.py \
  tests/compiler/passes/test_twisted_rewrite.py \
  tests/compiler/passes/test_schedule_walk.py -q
```
...
Fold lifting, classification, schedule selection, hardware materialization, source rendering, or runtime dispatch.
That locality is the central architectural advantage of Emmy's compiler stack.
