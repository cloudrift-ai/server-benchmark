...
description: >-
  A beginner-friendly, code-first course on Emmy's compiler, deployment, benchmarking, provisioning, and serving
  architecture.
...
`emmy.emmy:main`. Folders such as `recipes/`, `experiments/`, and `scripts/` contain inputs, durable evidence,
...
[From PyTorch to CUDA: Compiler Deep Dive](./compiler_deep_dive). It follows one operation through every IR, then
explains rewrite mechanics,
...
- The CLI is a thin coordination layer: it reads options and calls reusable code. Reusable behavior lives in
  `emmy/recipe`, `emmy/benchmark`, `emmy/deploy`,
...
./venv/bin/pytest tests/recipe/test_types.py tests/test_hardware.py -q
```

Use `rg --files emmy | sort` to compare the installed package with the top-level repository contents. Notice that
`recipes/`, `experiments/`, and `scripts/` are consumers or durable artifacts around the package.
...
- Which raw output files should be preserved?
...
planning, provisioning, deployment, benchmarking, and experiment-recording code.
...
flowchart LR
    ...
    Run --> Results[Preserve raw results and experiment records]
```
...
recipe path
  ...
  -> preserve raw results and update one system-only experiment record per row
```
...
1. `recipes/Qwen3-Embedding-0.6B/recipe.yaml`: a real inference recipe with `cross` and nested `zip` matrices.
2. `emmy/recipe/types.py`: `Recipe`, `LLMConfig`, `ModelConfig`, `CommandConfig`, and `DeployConfig`.
3. `emmy/recipe/matrix.py`: `expand_matrix`, `_expand_cross`, `_expand_zip`, `build_override`.
4. `emmy/recipe/recipe.py`: `deep_merge`, `_validate_and_build`, `load_recipe`, `resolve_for_hardware`.
5. `emmy/recipe/engines.py`: flag maps and `build_engine_args`.
6. `emmy/recipe/ARCHITECTURE.md`: use it to resolve edge cases after reading the implementation path.
...
- Which recipe sections describe the model or command, engine, hardware, workload, and comparisons.
...
flowchart LR
    ...
    Benchmark --> Save[Preserve raw results and update the experiment record]
...
Each expanded row has a typed `ExperimentRecord`. Emmy creates the record before provisioning and updates its lifecycle,
provenance, system information, timestamps, and failure details atomically as the row progresses. Measurements remain
raw artifacts: orchestration deliberately does not parse them into a second result schema or produce an interpreted
report. A `PhaseTimer` records orchestration phases without changing that ownership boundary.
...
```python
records = initialize_experiment_records(group.tasks)
record = None
...
try:
    ...
    for task in group.tasks:
        ...
        preserve_raw_artifacts(benchmark(task, host))
        update_experiment_record(record, state="succeeded")
        teardown_service(task, host)
except Exception as error:
    if record is not None:
        update_experiment_record(record, state="failed", error=error)
    raise
finally:
...
4. `emmy/benchmark/experiment_record.py`: the typed, system-only lifecycle and provenance record.
5. `emmy/benchmark/ARCHITECTURE.md`: artifact ownership and the experiment-record contract.
6. `emmy/timing.py`: `PhaseTimer` and phase constants.
7. `emmy/commands/ARCHITECTURE.md`: command ownership and orchestration boundaries.
...
./venv/bin/pytest \
  ...
  tests/benchmark/test_experiment_record.py \
  tests/benchmark/test_bench_status.py -q
```

Open `tests/benchmark/test_experiment_record.py` and compare the serialized identity and lifecycle fields with the
contract in `emmy/benchmark/ARCHITECTURE.md`. A real benchmark writes collision-safe `*.experiment.yaml` files beside
its raw artifacts; historical experiment directories may still contain older layouts and are not the current contract.
...
images, obtains model data, starts services, waits for health, runs a task-aware smoke test, and exposes teardown
through the same transport abstraction. Multi-instance configurations add nginx and use a scale-out strategy.
...
lifts every loop operation into a canonical pure `Fold` tree, classifies its algebra, exposes structural cuts, and then
attaches a **schedule**: the plan that assigns calculations and data movement to GPU hardware. A **tile** is a small
piece of a larger tensor calculation that can fit in faster memory.
...
The Loop-to-Tile boundary is a **total lift**: every loop node has a structural Tile representation, with no raw-loop
escape hatch. The Tile IR stores the pure `Fold`, placement, output specifications, and scheduling choices. Kernel
lowering binds the selected schedule to threads, blocks, shared memory, cooperative reductions, tensor-core atoms, and
synchronization. CUDA lowering renders the resulting kernel statement tree to source.
...
4. `emmy/compiler/ir/pure/`: the pure `Fold` algebra and classification vocabulary.
5. `emmy/compiler/ir/tile/`: `TileOp`, placement, output specifications, and Tile rendering.
6. `emmy/compiler/pipeline/passes/lowering/tile/010_lift.py`, `020_twisted.py`, `030_cut.py`, and `040_schedule.py`.
7. `emmy/compiler/ir/schedule/`: the generic `Schedule`, `ScheduleContext`, and classic schedule implementation.
8. `emmy/compiler/pipeline/passes/lowering/kernel/ARCHITECTURE.md`: materialization, factorization, and staging.
9. `emmy/compiler/ir/kernel/ir.py` and `render.py`.
10. `emmy/compiler/pipeline/passes/lowering/cuda/010_lower_kernelop.py`.
...
./venv/bin/pytest \
  ...
  tests/compiler/passes/test_cut_forks.py \
...
A **golden configuration** is one persisted symbolic program target. A reviewed canonical golden records one or more
realizations for that target on an exact GPU platform. Think of one verified realization as an approved benchmark card:

```text
For this exact program target and binding,
on this exact GPU,
...
| Field | Meaning |
|---|---|
| stable Torch IR and optional Loop IR | The exact program and target reconstructed for replay |
| `configs` and `realizations` | Symbolic targets plus concrete bindings and pin regimes |
| `gpu_name` and `compute_cap` | The GPU model and NVIDIA hardware capability on which it was recorded |
| `pins` and `knobs` | The input regime and every exact realized schedule choice, including explicit OFF |
| `measurements` | Reproduced Emmy and named-reference latency in microseconds |
  ...
  - program: 0
    ...
    realizations:
      - name: matmul.square.512.fp16
        ...
        knobs: {WORK: w2x1, TILE: mma_m16n8k16_f16_f32/f1x4/k2, RASTER: '', STAGE: d4/smem-async}
        measurements: {emmy_us: 8.3, reference_us: 6.0, reference_backend: cublas}
```

Model goldens live under each recipe's `golden/` directory; model-agnostic hardware goldens live under
`emmy/compiler/pipeline/search/goldens/`. A mutable working golden may contain inventory, proposals, and ranking
feedback, but only a reviewed canonical file is verified deployment evidence.
...
```mermaid
flowchart LR
    Shape[Create a working golden inventory] --> Tune[Autotune alternatives on the intended GPU at deployable O3]
    Tune --> Pin[Pin the exact knobs and run an A/B benchmark]
    ...
    Copy --> YAML[Promote the complete program-backed row]
    YAML --> Review[Run the file-scoped audit and qualification checks]
```

The important steps are:

1. Create a working golden inventory for the exact program, bindings, pins, and target GPU.
2. Run `emmy tune --golden-file` to explore valid schedules at the default deployable `-O3` settings.
3. Pin a candidate's exact knobs and compare it through the A/B path.
4. Require three integrity checks: the requested knobs were actually realized, the output matches the reference, and
   the reported speed is physically plausible for the GPU.
5. Repeat the measurement. Use the pinned row's `record_knobs` and latency, not a partial knob diff or an incomparable
   timing from another benchmark path.
6. Promote the complete program-backed record into the correct canonical per-GPU YAML and run its release audit.

A new candidate workflow has this general shape; the actual knob string comes from tuning output:

```bash
# 1. Search every target in a mutable working inventory.
...
./venv/bin/emmy run \
  ...
  --bench --warmup 5 --iters 20 \
  --json /tmp/matmul-golden-candidate.json
```
...
1. **Deployment-time schedule selection.** Greedy selection first checks for a verified record on the live GPU whose
   strict deploy identity matches the current schedule-free Loop program and whose schedule row equals one exact
   enumerated leaf. It is stronger evidence than ordinary tune measurements or model predictions.
2. **Reproduction and release checks.** `compile` and `run` accept `--golden NAME`; `tune` and strict multi-target runs
   accept a working `--golden-file`. `emmy eval golden FILE --serving-config FILE` performs the file-scoped release
   audit against a pinned serving realization matrix.
3. **Offline-prior fitting and evaluation.** `emmy fit` reconstructs each canonical golden's candidate pool to train
   the offline prior. Goldens do not enter the online reservoir, which learns from ordinary tuning measurements.
...
./venv/bin/emmy run \
  --golden matmul.square.512.fp16 \
  --bench --warmup 5 --iters 20 \
  --json /tmp/matmul-golden-reproduction.json

# Rank both prior halves on the canonical golden dataset.
./venv/bin/emmy eval prior --dataset golden --kernel matmul.square.512 --pool-sample 2000
```

A golden never forces an impossible or stale schedule. If its program identity matches but no live leaf exactly equals
the recorded schedule row, Emmy warns about drift and falls through to weaker evidence instead of replaying it
approximately. Goldens arbitrate only the deployable `-O3` lane and matching pin regime.

This gives the evidence order a useful shape:

```text
matching, realizable golden for this GPU
    -> compatible reservoir measurements
    -> compatible tune-DB perf measurements
    -> global Prior (online checkpoint with offline cold-start fallback)
    -> rule fallback option
```

The dedicated [How Emmy Chooses Fast GPU Schedules](./prior_and_goldens) chapter traces this hierarchy from a compiler
fork through feature construction, MCTS labels, persistence, live-card matching, and safe golden recording.
...
7. `emmy/compiler/pipeline/search/strategy/two_level.py`: structural outer search and per-operation inner tuning.
8. `emmy/commands/tune.py` and `emmy/commands/eval.py`: user-facing drivers.
9. `emmy/compiler/pipeline/search/golden.py`: generic program-backed canonical records and strict decode.
10. `emmy/compiler/pipeline/search/working_golden.py`: mutable inventories, proposals, and tune feedback.
11. `emmy/compiler/pipeline/search/policy/greedy.py`: verified evidence and the full hierarchy.
12. `emmy/commands/run.py`: pinned A/B checks, `record_knobs`, and machine-readable JSON output.
...
./venv/bin/pytest \
  ...
  tests/compiler/pipeline/search/test_two_level_strategy.py \
  tests/compiler/pipeline/search/data/test_freeze.py -q
```
...
You can distinguish a rule fork from a knob, exploration from greedy resolution, and offline from online priors. You
can also explain the verified, reservoir, DB, and prior tiers; what a program-backed golden stores; why promotion
requires repeated integrity-checked measurement; and why canonical goldens fit the offline half but not the online one.
...
first test: tests/recipe/test_types.py
...
| Why is a raw artifact or row record missing? | `benchmark/execution.py` | workload, experiment record, planner variant |
...
| Why was this GPU schedule selected? | total lift and scheduling | fork, knob, deploy evidence hierarchy |
