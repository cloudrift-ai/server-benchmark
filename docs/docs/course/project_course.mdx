...
## Lesson 2 — Turn recipe YAML into typed variants

### Learn

A recipe is a YAML configuration file that describes what to run and how to run it. YAML is a human-readable format
for nested lists and key/value settings. A recipe can also describe several experiments at once through its `matrices`
section. Each final combination is a **variant**.
