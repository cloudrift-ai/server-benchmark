...
`EmmyEmbedModel` is an attention-free vLLM pooling model. **Pooling** means that vLLM runs the model once over a
complete input, then reduces its token-level hidden states to one embedding vector. There is no next-token loop,
language-model head, or sampler. “Attention-free” describes the vLLM interface, not the transformer: the compiled
Emmy trunk still performs full-sequence attention, but it exposes no vLLM `Attention` layers and therefore needs no
autoregressive KV cache.

`EmmyGenModel` implements the generation contract instead. It repeatedly advances requests through prefill and
decode, exposes one vLLM `Attention` layer per decoder layer so vLLM can manage the paged KV cache, and hands the
result to vLLM's language-model head and sampler. In short, `EmmyEmbedModel` maps each input to one vector, whereas
`EmmyGenModel` produces a sequence of tokens over multiple decoding steps.

`EmmyEmbedModel` has no Torch parameters of its own because `EmmyForwardRunner` loads and binds the checkpoint
weights. Its `load_weights` method returns an empty set, so vLLM does not read a second copy of the checkpoint into
that wrapper.
...
Unlike the pooling model, `EmmyGenModel` is not attention-free. It creates one real vLLM `Attention` layer per decoder
layer so vLLM can discover the KV-cache requirements, allocate blocks, and run paged attention.
...
sequenceDiagram
    participant Client
    participant VLLM as vLLM
    participant Model as EmmyGenModel
    participant Runner as EmmyGenRunner
    participant Attention as vLLM attention and KV cache

    Client->>VLLM: POST /v1/completions
    VLLM->>VLLM: Tokenize and schedule
    loop Prefill, then decode steps
        VLLM->>Model: Scheduled token IDs and positions
        Model->>Runner: Embed token IDs
        loop Each decoder layer
            Model->>Runner: Run pre-attention program
            Runner-->>Model: q, k, v
            Model->>Attention: Apply RoPE and paged attention
            Attention->>Attention: Read and update KV cache
            Attention-->>Model: Attention output
            Model->>Runner: Run post-attention program
            Runner-->>Model: Next hidden states
        end
        Model->>Runner: Final normalization
        Runner-->>Model: Hidden states
        Model-->>VLLM: Hidden states
        VLLM->>VLLM: Compute logits and sample
    end
    VLLM-->>Client: Completion response
```
...
The generation adapter is architecture-aware. It builds per-layer attention and rotary metadata for mixed local and
global attention, supports routed-expert models through the MoE third seam, and can shard layers and experts across
vLLM tensor and pipeline ranks. Current serving paths include compressed expert formats and the published DeepSeek V4
checkpoint; uniform sliding-window and dual-chunk configurations remain rejected. A clean startup error is safer than
producing plausible but incorrect tokens.
...
running throughput experiments. `scripts/validate_serve.py` is the sequential GPU accuracy gate used by the serving
image workflow; it compares a checkpoint's Emmy server with a Hugging Face eager reference without keeping both models
resident at once.
