...
def _emit_offline_eval(kernel_filter: str | None) -> None:
    ...
    for g in configs:
        # The retired WSPEC key drops so the stored row compares fact-for-fact with a realized one.
        gold = ingest_row(dict(tuning_knob_items(g.knobs)))
        try:
    ...
    try:
        for name, group in groups.items():
            for fm in (False, True):
                ...
                from emmy.compiler.pipeline.knob import ingest_row  # noqa: PLC0415

                golds = [ingest_row(tunable(c.knobs)) for c in sub]
