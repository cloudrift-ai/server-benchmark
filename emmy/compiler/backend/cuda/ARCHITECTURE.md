...
`want_ref` (return that run's `(inputs, outputs)` as `run_io`). Embedded Loop replay has no Torch
twin, so its Emmy-only greedy execution returns that same-input reference too. If this execution
completes but the later repeated greedy timing crosses the watchdog, the worker returns the reference,
its single-run event timing, and the exact timing error. The command marks greedy ineligible while still
reference-checking pinned rows; the parent retires that child before any pinned job, so a still-running
kernel cannot share its context. It never raises the watchdog or treats the single run as a candidate.
Rebuilding the torch side **in the child** (not pickling a live module) is what lets the interleaved comparison — which
couldn't cross a subprocess boundary before — run isolated. So `tune --bench` (`commands/tune.py` `_run_bench` /
