...
| Storage-decode | `TrellisDecodeOp`, `HadamardOp` — the ONE checkpoint-shaped exception, and only because the hat-basis form has an in-kernel realization (see `compiler/ARCHITECTURE.md`). No decomposition rule: the checkpoint-basis form appears only inside constant-only cones the EXL3 speller mints, which `032_fold_constant_subgraphs` collapses into bind-time `source_graph` records (its numpy `forward` is the record's evaluation); the hat-basis form lifts to `TrellisLoad` reads instead. The alternative `EMMY_TRELLIS_SPELLING=generic` speller (`loader/trellis.py`) emits no op from this row at all. |
...
`RangeOp`, `CastOp`, and `BitcastOp` are generic value operations, not checkpoint-format operations. Their only
consumer today is the `EMMY_TRELLIS_SPELLING=generic` reconstruction algebra, so `032_fold_constant_subgraphs`
removes them before Loop lifting. A future runtime consumer may add ordinary lifting for the same semantics without
changing checkpoint ingestion.
