...
3. **Storage-decode ops** — ``TrellisDecodeOp``. Its frontend decomposition
   collapses its constant-only cone into a bind-time ``source_graph`` record,
   where its numpy ``forward`` runs through the reference NumPy backend.
"""
...
class TrellisDecodeOp(Op):
    ...
    record's evaluation through the reference NumPy backend. This frontend fold is its only
    decomposition; no lowering rule exists, and the op must never survive into Loop IR.
    """
