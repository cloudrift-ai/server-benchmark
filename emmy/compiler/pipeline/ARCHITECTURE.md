...
as a loop-level TAP in its producer: `loop/fission/010_tap_row_stat` fuses it unconditionally at merge-fixpoint, the
recognition peel keeps the host's untapped identity, and the decision rides the tapped PRODUCER's fork — `sink`
keeps the tap (`030_split_reduce` relocates it onto a split's finalize; `034_attach_taps` materializes the
tier-derived fold, `RowAccum` on the cross-CTA tier) while option-0 `fuse` cuts it back out
(`015_cut_stat_tap` — the sweep re-welds its statistic and re-enters `010`, landing on the local coop norm
byte-identically to the never-fissioned compile). Precedence
...
line; a `sink` whose settled row can't host the tap (an atomic split-K partial, an mma epilogue) degrades to the
cut-out — exactly the fuse kernels. Since `@` is not a valid shell var character, per-element pins
ride `EMMY_KNOBS` (e.g. `EMMY_KNOBS="PLACE@fold=cut"`); bare `EMMY_PLACE` pins every eligible edge. Never enumerated
as a search dimension — but `stat`'s `sink` rows are offered as evidence-only fork SIBLINGS (mirroring every row of
the tapped producer's fork), selectable by a recorded golden exactly like `PLACE@cone: cut`.
...
| `loop/fission/`           | `tap_row_stat` — the row-statistic TAP fission (the `PLACE@stat` resting-state inversion): once fusion's fixpoint has fully assembled a standalone norm over an in-graph producer, its statistic fuses INTO the producer as an atomic-accumulate tap (`ir/loop/tap.py`, output slot 1 `T__sq`) and the norm drops to a wide sweep reading it. Its own pass, deliberately: a pass advances only at a quiescent scan, so mid-assembly forms (a softmax one merge short of its flash offer site) can never fission. The tile passes realize the decision (`015_cut_stat_tap` / `030_split_reduce` relocation / `034_attach_taps`). |
...
| `loop/stamp/`             | `stamp_loop_names` (`provenance.name_for`, e.g. `k_rms_norm_3f2a1b`) + `stamp_structural_features` (the `S_*` dict). Runs last in the loop dialect — after fusion and fission — so every kernel is named / stamped against its final body. Both stamps are TAP-BLIND (`ir/loop/tap.strip_taps` before hashing/featurizing): a tapped producer keeps its untapped kernel name and fork identity, the contract the recognition peel and the golden joins rest on. |
