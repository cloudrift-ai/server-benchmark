...
| `search/data/` | The harmonized read-view over the three data sources (golden records / DB `perf` rows / prior reservoir): `Sample`, `Dataset`, the derived `ShapeKey` index, and `group.py`'s `Group` — one candidate pool packed as a matrix plus ONE label array, whose `label_kind` is `pinned` (which rows a golden verified) or `latency` (what each row measured), fixed by the builder that made it. `group_measured` builds the latency kind from benched node rows. Nothing here imports `search/prior/`: a group carries every column it was given, and each model class narrows to the ones it wants when it asks for the matrix — `TREE_FEATURES`, the view argued entirely from what a tree can re-derive, lives with the CatBoost trainer for the same reason. |
...
- **A case is a candidate pool, and it may have more than one right answer.** `Group.pinned` — the `pinned` kind
  of `Group.labels`, read back as an index set — is the set of rows in that pool a golden verified: usually one,
  several when the builder matched several goldens onto one pool (the same shape recorded under two names, or one
  name recorded twice). The per-case term is then the BEST rank over
