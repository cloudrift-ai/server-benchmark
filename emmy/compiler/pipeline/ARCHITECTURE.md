...
| `search/cost_model.py` | The fitted estimate of a kernel's best achievable latency, plus the artifact that ships it and the cross-validated report that says whether it is good enough to decide a fork. Deliberately NOT a `Prior`: a prior ranks candidates INSIDE one pool and its score is ordinal, this prices a whole pool in absolute microseconds so a structural fork can sum it. Regresses `log(measured / roofline)` — the floor spans the corpus, the fit places a kernel above it. |
| `search/kernel_cost.py` | What ONE kernel costs at best, and the features that predict it: compulsory traffic, stamped work, the physical floor under them (`max(arithmetic, data movement, launch)`), and the feature row built from all three. Answers the question a `Prior` cannot — a prior's score is ordinal and ranks only *within* one pool, while a structural fork compares kernel SETS and needs absolute microseconds to sum. Everything here is a function of the kernel and the card and never of the fork that produced it, so the same kernel prices identically whether a placement cut minted it or it arrived standalone. |
