...
- **A case is a candidate pool, and it may have more than one right answer.** The `verified` kind of
  `Group.labels` marks the rows in that pool a golden confirmed: usually one, several when the builder matched
