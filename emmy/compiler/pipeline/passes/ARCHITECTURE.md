...
split-K is the redundant-statistic form (the k-invariant prologue stays full-row in every partition, only the per-cell
cone σ-reindexes), and the cone's own statistic site is a nested site under the same inventory — the nested site is
why the enumerator recurses. The compute fill is MANDATORY for a computed edge and for a multi-channel product, with
ONE exception: the packed-pair (NVFP4) weight cone, which additionally offers the `smem-async` / `smem-tma` byte-slab
rows as fork siblings — its bits copy verbatim and only its block scales are compute-filled. The exception reads the
node's own shape, never the pin's transport, so a multi-channel product carrying an async or TMA pin still takes the
fill: the byte-transport emitters are single-channel. A `STAGE` pin picks between the two families by its transport.
SDPA carries no family of its own: its two matmuls schedule as plain contractions and the online-softmax `TWISTED`
fold takes the ordinary reduce partition. A term the
