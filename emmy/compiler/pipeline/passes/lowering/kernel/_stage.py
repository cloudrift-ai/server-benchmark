...
class SyncTransport:
    ...
    def slab_decls(self, ring: int) -> list[Stmt]:
        ...
        # Copied peer slabs allocate the ring. Each is sized by its OWN element width and row pad
        # — a copied peer here is a copied operand there (a packed-pair weight's byte slab beside
        # an f16 A), so a byte slab keeps its 16 B alignment and its padded rows.
        return [
