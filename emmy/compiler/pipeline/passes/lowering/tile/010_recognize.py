...
schedule fork exists, so it cannot wait for ``020``.) Every ``LoopOp`` arrives already carrying its
``S_*`` structural identity — the ``IdentityStrategy`` stamps fusion-born kernels at the loop
dialect's end and minted pieces at the splice event, so recognition never has to order itself
against a stamp.
...
def rewrite(match: Match, root: Node, ctx=None) -> TileOp | Graph | None:
    loop: LoopOp = root.op
    # A kernel is RECOGNIZED only once it has an identity. Every ``LoopOp`` reaching this rule
    # was stamped with its ``S_*`` structural identity at birth by the ``IdentityStrategy`` —
    # fusion-born kernels at the loop dialect's end, minted pieces (a cut's fragments, a split's
    # pieces) at the splice event, before the fragment even entered the graph. No ordering guard
    # is needed here: no rule, whatever the cursor position, can observe an unstamped kernel.
