...
from emmy.compiler.ir.tile.ir import ContractionView, Fold, Map, _operand_name, effect_tail
...
def _ancestor_axes(root, child) -> tuple[Axis, ...]:
    ...
    def walk(node, acc: tuple[Axis, ...]) -> tuple[Axis, ...] | None:
        ...
        if isinstance(node, Map):
            ...
            edges = ()
        below = (*acc, node.axis) if isinstance(node, (Fold, ContractionView)) else acc
        for e in edges:
            if isinstance(e, (Map, Fold, ContractionView)):
                got = walk(e, below)
                if got is not None:
                    return got
        return None
...
def _ws_dtype(child, inputs: dict):
    ...
    if isinstance(child, (Fold, ContractionView)):
        return F32
    for s in lower(child):
...
def _replace_edge(node, child, load: Load):
    ...
    _nodes = (Map, Fold, ContractionView)
    if isinstance(node, Map):
