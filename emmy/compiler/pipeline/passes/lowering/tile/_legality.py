...
The multiplicative rules — a thread budget, a K-step that must tile a static extent, a 16 B-aligned
inner stride — are :class:`~emmy.compiler.pipeline.search.domain.Bound`\\ s built from the node, so
they are stated in the same currency the candidate DOMAIN is generated in
(:mod:`~emmy.compiler.pipeline.search.space`). The categorical rules (an operand dtype, a transport,
a fragment-unrealizable epilogue) are plain predicates: no coordinate change makes a dtype
multiplicative, and pretending otherwise would only obscure them.
...
from emmy.compiler.pipeline.search.domain import Bound
...
def _divides(coeff: int, limit: int) -> bool:
    """Whether ``coeff`` divides ``limit`` — the one-dimensional ``divides`` bound, evaluated at the
    degenerate point where the coefficient carries the whole product."""
    return Bound(("_",), limit=limit, op="divides", coeff=coeff).holds(coeff, complete=True)


def _at_most(product: int, limit: int) -> bool:
    return Bound(("_",), limit=limit).holds(product, complete=True)
...
def scalar_block_threads(plan: TilePlan) -> str | None:
    ...
    if _at_most(plan.units_n * plan.units_m, MAX_BLOCK_THREADS):
        return None
    return (
...
def strip_width(extent: int, width: int) -> str | None:
    ...
    if width <= 1 or (extent and _divides(width, extent)):
        return None
...
def _warp_cp_async(k_axis: Axis, tile_n: int, bk_elems: int, mask_n: bool, b_trans: bool) -> bool:
    ...
    if mask_n or not k_axis.extent.is_static:
        return False
    if not _divides(bk_elems, k_axis.extent.as_static()):
        return False
    return _divides(_CP_ASYNC_MIN_ELEMS, bk_elems) and (b_trans or _divides(_CP_ASYNC_MIN_ELEMS, tile_n))
...
def _warp_tma(k_axis: Axis, n_axis: Axis, tile_n: int, bk_elems: int, elem_bytes: int, mask_n: bool, b_trans: bool) -> bool:
    ...
    if not _divides(bk_elems, k):
        return False
    inner = (bk_elems, k) if b_trans else (bk_elems, tile_n, k, n)
    return all(_divides(_TMA_ALIGN, x * elem_bytes) for x in inner)


def resolve_warp_stage(c: Fold, tile: TilePlan, stage: Stage, budget: int = STATIC_SMEM_CAP) -> Stage | None:
    ...
    box_ok = _at_most(max(m.tile, n.tile, bk_elems), _TMA_MAX_BOX)
...
def resolve_scalar_stage(c: Fold, tile: TilePlan, stage: Stage, inputs, budget: int = STATIC_SMEM_CAP) -> Stage | None:
    ...
    if tile.launch_threads is None:
        return None
    if stage.transport == "tma" and not _at_most(max(tile.m.tile, tile.n.tile), _TMA_MAX_BOX):
        return None
    ...
    if not n_ext.is_static or not (_divides(_TMA_ALIGN, k * elem_bytes) and _divides(_TMA_ALIGN, n_ext.as_static() * elem_bytes)):
        return None
    ...
    while depth >= 1:
        ...
        bk_elems = next((v for v in (128, 64, 32, 16, 8, 4) if v <= cap and _divides(v, k)), 0)
        if bk_elems >= 4:
            break
        depth -= 1
    if bk_elems < 4:
