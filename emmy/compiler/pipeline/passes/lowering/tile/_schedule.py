...
def _with_f16acc(atoms: tuple[str, ...], ctx) -> tuple[str, ...]:
    ...
    f16acc forks are ordinary ``TILE`` rows (identified by the value's atom token, priced by the
...
# site-local (``spell``) and factor the worker geometry — the tile's unit widths, a coop
...
def _reduce_specs(kernel, place, ctx=None) -> list[ReducePlan]:
    ...
    # coalesced on ONE B orientation — the plain band interleaves lanes along K (fast only when K is
    # B's fastest-varying stride, the serving ``F.linear`` layout), the transposed band sweeps
    ...
    # 10-100× incidents in one day: qk_global_cat/gate_up m256 ``.lin`` on the transposed band, the
...
def _primary_fold(op) -> Fold | Contraction:
    ...
    :class:`Contraction` of a contraction kernel."""
    if isinstance(op, (Fold, Contraction)):
        return op
    assert isinstance(op, Map) and len(op.sources) == 1 and isinstance(op.sources[0], (Fold, Contraction)), (
...
def _option(tile, place, plan: ReducePlan, name: str, knobs: dict, reduce_key: str | None = None) -> TileOp:
    ...
    out = TileOp(op=tile.op, name=name, place=place, knobs={**knobs, rkey: plan.spell()}, stores=tile.stores)
    sched = sched_of(out)
    if plan.stages:
...
def _warp_atoms(kernel, con: Contraction | None, proj: Body | None = None) -> tuple[str, ...]:
    ...
    Takes the STORED node: every read here is algebra (the operand edges +
...
def _demote_mixed_a(kernel, con: Contraction | None) -> Contraction | None:
    ...
    algebra (the ``a`` edge), so the caller's placed ``TilePlan`` is unaffected."""
...
def _stage_candidates(
    kernel, con: Contraction | None, axes, plan: TilePlan, budget: int = STATIC_SMEM_CAP, tma_ok: bool = True
) -> list[str]:
    ...
    if con is None or not plan.is_tiled:
    ...
    def resolve(spec: str) -> str | None:
        ...
        r = _resolve_warp_stage(con, tile, st, budget) if plan.is_warp else _resolve_scalar_stage(con, tile, st, kernel.inputs, budget)
...
def _tile_rows(kernel, place, ctx=None) -> tuple[list[dict], tuple[str, str, str]]:
    ...
    try:
        con, cell, proj = contraction_view(kernel.op, place, TilePlan())
    except LoweringError:
        con, cell, proj = None, TilePlan(), Body(())
    axes = cell.axes or ()
    if con is not None and con.a_computed:
        rows = _computed_a_rows(kernel, place, con, axes, keys, _smem_budget(ctx), ctx, proj=proj)
        return _filter_work(rows, lambda r: r.get(WORK.name, "")), keys
    tiles = scalar_tile_moves() if con is not None else [TilePlan()]
    warp_offered = False
    ...
    if con is not None:
        ...
        if atoms:
            ...
            if demoted is not con:
                # The rewritten algebra keeps the SAME placement.
                demoted_rows = _computed_a_rows(kernel, place, demoted, axes, keys, _smem_budget(ctx), ctx, proj)
    ...
    for plan in tiles:
        if plan.is_warp and pin is not None:
            ...
            if con is not None and not _fragment_epilogue_ok(proj):
                raise ValueError(
        ...
        for stage in _stage_candidates(kernel, con, axes, plan, _smem_budget(ctx), _tma_allowed(ctx)):
            for red in _reduce_candidates(kernel, place, plan, con, len(con.channels) if con else 1):
...
def _resolve_warp_stage(c: Contraction, tile: TilePlan, stage: Stage, budget: int = STATIC_SMEM_CAP) -> Stage | None:
...
def _resolve_scalar_stage(c: Contraction, tile: TilePlan, stage: Stage, inputs, budget: int = STATIC_SMEM_CAP) -> Stage | None:
    ...
    if stage.transport == "tma" and not (
        _tma_operand_rank_ok(c.a.index, tile.m.axis.name, c.k_axis.name)
        and _tma_operand_rank_ok(c.b.index, tile.n.axis.name, c.k_axis.name)
    ):
        return None
...
def _resolve_sync_stage(c: Contraction, tile: TilePlan, budget: int = STATIC_SMEM_CAP, want_depth: int = 1) -> Stage | None:
    """The ``sync`` compute-fill :class:`Stage` for a **computed-A** warp contraction with tile plan
    ``tile`` — MANDATORY for this form (the gmem-direct mma leaf refuses a computed A; cp.async /
...
def _computed_a_rows(
    ...
    if pin is not None:
        ...
        if pin.at(*axes).n.mask:
            raise ValueError(
                f"warp TILE pin on a fused-cone contraction: the tile's N width must exactly cover the static "
                f"output columns (N={axes[1].extent}, no N mask in the sync compute-fill); pick a dividing tile."
            )
        tiles = [pin]
    else:
        atoms = _with_f16acc(_warp_atoms(kernel, probe, Body(tail)), ctx)
        tiles = [p for p in warp_tile_moves(atoms) if _warp_move_ok(kernel, p) and probe.k_axis.extent.is_static and not p.at(*axes).n.mask]
    ...
    for plan in tiles:
        ...
        for want_depth in depths:
            stage = _resolve_sync_stage(probe, plan.at(*axes), budget, want_depth)
            if stage is None:
                if pin is not None:
                    raise ValueError(
                        f"warp TILE pin {plan.spell()!r} on a fused-cone contraction: the sync slabs exceed the {budget} B smem budget."
                    )
                continue
...
def _splitk_option(
    ...
    inner, inner_t, epi = contraction_view(tile.op, place, wt)
    w = rplan.cta
    ...
    if inner.a_computed:
        ...
        stage = _resolve_sync_stage(inner, inner_t, budget, Stage.parse(stage_spec).depth if stage_spec else 1)
        if stage is None:
            raise ValueError(
                f"split-K on a fused-cone contraction: the sync slabs exceed the {budget} B smem budget at TILE {wt.spell()!r}"
            )
    elif stage_spec:
        st = Stage.parse(stage_spec)
        stage = (
            _resolve_warp_stage(inner, inner_t, st, budget)
            if wt.is_warp
            else _resolve_scalar_stage(inner, inner_t, st, tile.inputs, budget)
        )
    ...
    stamped = {**knobs, tkey: wt.spell(), rkey: rplan.spell()}
...
def _warp_option(
    ...
    so materialize only ``factorize``\\ s. The stamped dict spells the SITE grammar (``_site_knobs`` +
    ...
    node, node_t, proj = contraction_view(tile.op, place, wt)
    node = _demote_mixed_a(tile, node)  # the algebra may change; the placed tile is untouched
    ...
    if node.a_computed:
        stage = _resolve_sync_stage(node, node_t, budget, Stage.parse(stage_spec).depth if stage_spec else 1)
        assert stage is not None, "computed-A row enumerated past its smem budget"  # _computed_a_rows resolved this
    else:
        stage = _resolve_warp_stage(node, node_t, Stage.parse(stage_spec), budget) if stage_spec else None
    ...
    workers, _ = _wspec_workers(wspec_spec, stage, node_t.launch_threads)
    ...
    stamped[skey] = stage.spell() if stage is not None else ""
    # The WSPEC key never stamps: the resolved producer band rides ``TileOp.workers`` and
...
def _tile_option(
    ...
    if plan.is_tiled:
        ...
        rplan = None
        try:
            node, node_t, proj = contraction_view(tile.op, place, plan)
        except LoweringError:
            ...
            if stage_spec:
                stage = _resolve_scalar_stage(node, node_t, Stage.parse(stage_spec), tile.inputs, budget)
            fold = node
            op = Map(body=proj, sources=(fold,)) if len(proj) else fold
            slices = [("TILE", fold, node_t), ("STAGE", fold, stage)]
...
def _map_strip_option(tile: TileOp, place: Placement, inner: Axis, plan: TilePlan, name: str) -> TileOp:
    ...
    return TileOp(op=Map(body=body), name=name, place=new_place, knobs={TILE.name: plan.spell()}, stores=tuple(stores))
...
def schedule(tile: TileOp, name: str, knobs: dict, ctx=None, reduce_key: str | None = None) -> Fork | list[TileOp] | TileOp:
    ...
    # (a leading atom token — ``TilePlan.is_warp``) builds the tensor-core warp option, otherwise the
    ...
    if role is AxisRole.CONTRACTION:
        ...
        def _materialize(row: dict) -> TileOp:
            # The row carries SITE values + the ONE ``WORK`` entry. Resolve each family
    ...
    if rplan is not None:
        if rplan.needs_split and rplan.finalize == "kernel":
            ...
            if pair is not None:
                red, head, pv, head_ax, pv_ax = pair
    ...
    if not (REDUCE.raw() or "").strip():
        pair = twisted_pair(tile.op, tile.place.free)
        if pair is not None:
            red, head_f, pv_f, head_ax, pv_ax = pair
            head, pv = head_f, pv_f
            warps = _twisted_warp_options(tile, name, knobs, _smem_budget(ctx), _tma_allowed(ctx), _f16acc_allowed(ctx))
            # A live **warp** ``TILE`` pin (an atom-naming value) OR a non-empty ``STAGE`` pin keeps the mma
...
def _canon_tile_spec(spec: str) -> str:
    """A ``TILE`` spelling in the site codec's normal form (``a:scalar`` ≡ ``""``, ``f64x1`` ≡
    ``f64``) so a pin-only alias compares equal to the row's stamped value; an unparseable pin
    passes through (it just won't match, and :func:`_narrow_flash_forms` degrades gracefully)."""
...
def _narrow_flash_forms(forms: list[TileOp], head: Contraction, pv: Contraction, *, keyed_only: bool = False) -> list[TileOp]:
    ...
    if not live:
        return forms
    # The rows spell SITE values, so the pin selects by value alone; the shared warp geometry is
    # the ``WORK`` family's business (``_filter_work``, which every caller applies around this).
    kept = [f for f in forms if all(_canon_tile_spec(f.knobs.get(f"{TILE.name}@{ax}", "")) == site for ax, site in live.items())]
    if not kept:
...
def _demoted_warp_option(tile: TileOp, place, name: str, knobs: dict) -> TileOp | None:
    ...
    stamped = _site_knobs({**knobs, _family_key(emitted, TILE.name, fold): wt.spell()})
...
def _twisted_warp_options(
    ...
    pv_atoms = [atom]
    # The sibling-atom name is a substring of the pin — the value's bare leading ``<atom>/…``.
        ...
        for stage in stage_cands:
            ...
            for wspec_cand in wspecs:
                ...
                for pv_plan in variants:
                    # SITE spellings: the shared warp geometry lives once in the WORK entry
