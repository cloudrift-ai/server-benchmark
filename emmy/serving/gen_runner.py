...
def _plan_sources(plan, wrapper, np_dtype, ckpt_dir, id_to_key):
    ...
    unbindable = [
        nid
        for nid, w in plan.weights.items()
        if w.load_ops is None or (w.source_path is None and not w.source_parts and w.source_op is None)
    ]
    if unbindable:
        raise RuntimeError(
    ...
    **prefill** program (one program, any width). ``argnames=None`` traces a **fully static**
    ...
    if plan is None:
        ...
        if ckpt is not None:
            ...
            spell_trellis_constants(graph, ckpt[0], expand=True)
        if quant_specs:
    ...
    def from_model(cls, model, *, dtype_str="float16", decode_bucket=16, max_tokens=None, prefill_bucket=0, expert_store=None):
        ...
        def _build_expert_group(g, experts, einputs, expert_w, expert_fmt, trellis, has_bias, layer):
            ...
            with torch.device("cpu"):
                ...
                # `_indirect_covered` and recompiles — no half-hit. The shared Hadamard of an
                # EXL3 group is a graph CONSTANT, not a per-expert input, so it never enters a
                # table.
