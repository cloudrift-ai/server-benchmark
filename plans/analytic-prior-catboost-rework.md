...
6. **Tier-2 additive fallback.** Depth-1 fit stage + declared pair terms (atomic-free × split-width, plus decision
   ...
   these the additive table is arch-blind by construction), export to source-literal table, skip-if-missing scorer;
   delete `_W_A` / `_W_A_DYN` once parity is shown. Verify: suite ≥ linear baseline on goldens **per card** (a
