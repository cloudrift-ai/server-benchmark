...
**Resolved (mlp_down split-K):** the K-heavy `mlp_down` GEMM (M=512, N=4096, **K=14336**) trailed cuBLAS ~0.85×
because a 128-CTA grid at 33% occupancy under-parallelized the long K axis (NCU: latency-bound on barrier +
short-scoreboard stalls, *not* memory or bank conflicts). Mimicking cuBLAS's low-occupancy / deep-pipeline recipe
made emmy *slower*; the fix is the opposite — **8-way split-K (`g8k`) on a smaller high-occupancy tile
...
**Resolved (rms_norm / softmax):** `rms_norm` (0.25–0.37×) / `softmax` (0.32–0.44×) trailed torch's fused norms
because the goldens deployed `REDUCE=b32` — one warp reducing *and* normalizing an entire K-row (NCU: 9.3% of peak DRAM
...
**Resolved (attention re-bench flatten):** the recorded attention goldens re-benched 200–1000× slow
(`hd128` 9609 µs, `hd64.dynM` 3103 µs) — a **golden-recording flatten**, not compile fragility. Flash is two
contractions (Q@Kᵀ, ·V), and the goldens stored a single **bare** `TILE`; on re-bench it collapses onto both axes
and, when they need different tiles, decays to a scalar fallback. Re-recorded each shape in the ONE form that
reproduces (verified by re-bench): `hd128` per-axis `TILE@dd`/`TILE@pj` → 16.5 µs; `hd64` a bare `TILE` → 9.6 µs
(the two forms genuinely deploy different kernels — flash pin resolution is inconsistent); the `.dynM` twins a bare
`TILE` (their axis-keyed pin never resolves) → 19.5 / 29.6 µs. Guard added
(`AttentionGoldenConfig.__post_init__`): a dynamic attention golden with an axis-keyed `TILE@` fails the schema.
The residual dynamic-flash gap is #2; the deeper flash-pin inconsistency (bare vs `TILE@<axis>` deploy different
kernels; dynamic never resolves axis-keyed) is a standing compiler bug worth its own fix.
...
| attention.hd64 | sta | 9.60 | 10.24 | 1.07 | |
| attention.hd128 | sta | 16.50 | 18.42 | 1.12 | |
| attention.hd64.dynM | dyn | 19.50 | 10.23 | 0.52 | #2 |
| attention.hd128.dynM | dyn | 29.60 | 18.40 | 0.62 | #2 |
...
## 2 — dynamic flash trails static (masked streaming flash residual)
...
fallback (the flash goldens already use MMA); it is genuine masked-streaming-flash work. `hd128.dynM` tops out at
**0.62×** (29.6 µs) and `hd64.dynM` at **0.52×** (19.5 µs) even after a TILE sweep — the wrong K-chunk (`k16` on
hd128, `k8` on hd64) falls to a scalar path, so this is a real residual, not a config miss. **Next:** profile the
masked-streaming-flash inner loop vs the static twin.

## 3 — causal attention deadlocks the bench worker on sm_120
...
- Golden rows that bench >5× their recorded `emmy_us` should be flagged `⚠ pathological re-bench`, not silently
  tabulated — better, a GPU-gated test should re-bench every golden and assert it reproduces its recorded
  `emmy_us` within a factor (that would have caught the flatten below at commit time, not by hand).
