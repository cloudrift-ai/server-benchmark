...
| compiling | `emmy compile nvidia/Qwen3-8B-NVFP4 --layer 0 --seq-len 512 --ir tile` | at a prefill width the pin names the block-scaled atom `mma_m16n8k64_e2m1_f32`, with **both** operands `f4e2m1x2` — the activation-quantize step visible ahead of the matmul | **OBSERVED** — layer-0 compile on the 5090: `Fold[a2 in 0..4096] contraction ⟨TILE=mma_m16n8k64_e2m1_f32/f1x8/k8 STAGE=d1/smem-async⟩` on `v_proj`, both operand edges packed decode chains. Three of the layer's six matmul kernels reach it — `v_proj` from stored codes, `o_proj` and `down_proj` from codes their own kernel computes into the slab. The other three do not: `q_proj`/`k_proj` never bind at all (their trailing free axes are head/head_dim, a pre-existing limit in Known gaps), and the fused `gate`+`up` kernel is a two-channel node, which the pair reading declines |
| compiling | `emmy compile nvidia/Qwen3-8B-NVFP4 --layer 0 --seq-len 512 --ir cuda` | emitted CUDA carries the PTX `mma.sync.aligned.m16n8k64...kind::mxf4nvf4.block_scale...ue4m3`, both multiplicands e2m1 | **OBSERVED** — the emitted kernel carries `emmy_mma_m16n8k64_e2m1_f32(_c0_[j], _a0, _b{j}, _sfa0, _sfb{j})` over four verbatim byte slabs, with `emmy_mma_load_sf{a,b}_f4` supplying the scale registers and no `EMMY_F4_LUT` on either operand. Parity vs numpy on a direct-fed activation (no computed producer, so both backends encode identical codes): median 1.8e-5, max 5.6e-4 relative — the fused-scale gap decision 18 accepts, not the exact oracle |
