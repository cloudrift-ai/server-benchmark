...
def test_trace_inventory_lists_each_sdpa_split_kernel_as_a_target(tmp_path) -> None:
    """SDPA lowers through the generic split (Q·K^T | softmax pieces | P·V), so the inventory
    carries one loop-IR target per split kernel — cut pieces have no frontend cone of their own,
    so no ``origins`` mapping, and the kernel name keeps the sdpa provenance."""
    ...
    assert [record.loop_index for record in records] == [0, 1, 2]
