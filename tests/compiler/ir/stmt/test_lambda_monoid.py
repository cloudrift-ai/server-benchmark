"""The λ-foldMap primitives (1m): :class:`Lambda` formation + α-invariance, :class:`Monoid` —
...
from emmy.compiler.ir.stmt import Accum, Assign, Body, Init, Lambda, Load, Loop, Monoid, Write
from emmy.compiler.ir.stmt.algebra import Carrier, State, Twist, eval_lambda, foldmap_eval
...
def test_monoid_of_is_degenerate_with_identity_seeds() -> None:
    m = Monoid.of("add", "maximum")
    assert m.arity == 2
    assert m.init == (0.0, -1e30)
    ops = m.component_ops()
    assert ops is not None and [o.name for o in ops] == ["add", "maximum"]
    assert m.degenerate


def test_monoid_arity_mismatch_rejected() -> None:
    lam = Lambda(params=("a", "b"), body=Body((Assign(name="c", op="add", args=("a", "b")),)), results=("c",))
    with pytest.raises(ValueError, match="S × S → S"):
        Monoid(init=(0.0, 0.0), combine=lam)


def _lse_monoid(n_expect: int = 0) -> Monoid:
    """The exp/LSE-family monoid at arity ``2 + n_expect`` — combine generated by the carrier
    machinery (``exp_combine_states``), wrapped as the ONE stored program."""
    ...
    return Monoid(init=(float("-inf"), 0.0) + (0.0,) * n_expect, combine=combine)


def test_lse_monoid_is_not_degenerate() -> None:
    m = _lse_monoid()
    assert m.component_ops() is None
...
@pytest.mark.parametrize(
    "monoid",
    [Monoid.of("add"), Monoid.of("maximum"), Monoid.of("add", "add"), _lse_monoid(0), _lse_monoid(1)],
    ids=["add", "max", "add2", "lse2", "lse3"],
)
def test_combine_is_associative_on_random_states(monoid: Monoid) -> None:
    rng = np.random.default_rng(7)
    n = monoid.arity
    for _ in range(50):
        a, b, c = (tuple(rng.normal(size=n) * 3.0) for _ in range(3))
        lhs = eval_lambda(monoid.combine, (*a, *eval_lambda(monoid.combine, (*b, *c))))
        rhs = eval_lambda(monoid.combine, (*eval_lambda(monoid.combine, (*a, *b)), *c))
        np.testing.assert_allclose(lhs, rhs, rtol=1e-12, atol=1e-12)


def test_init_is_the_neutral_element() -> None:
    rng = np.random.default_rng(3)
    for monoid in (Monoid.of("add"), Monoid.of("add", "add"), _lse_monoid(1)):
        s = tuple(rng.normal(size=monoid.arity))
        left = eval_lambda(monoid.combine, (*monoid.init, *s))
        right = eval_lambda(monoid.combine, (*s, *monoid.init))
...
def test_agreement_square_statistic() -> None:
    ...
    spec = foldmap_eval(Monoid.of("add"), lift, [(k, x[k]) for k in range(12)])
    np.testing.assert_allclose(got, spec[0], rtol=1e-12)
...
def test_agreement_dot_product() -> None:
    ...
    spec = foldmap_eval(Monoid.of("add"), lift, [(k, a[k], b[k]) for k in range(8)])
    np.testing.assert_allclose(got, spec[0], rtol=1e-12)
