"""The sympy bridge — ``symbolic`` / ``equal`` / ``factor`` / ``linearize``.

``Expr`` is syntax, sympy is semantics. These tests state what each reading DECIDES; nothing here
builds IR from a normalized expression, which is the bridge's one-way rule.
"""
...
from emmy.compiler.ir.symbolic import equal, factor, linearize, symbolic
...
def test_the_attention_summand_splits_into_its_invariant_and_varying_factors() -> None:
    ...
    split = factor(body, "q", lambda name: name in {"w", "v"}, _hoist)
...
def test_a_square_is_listed_twice() -> None:
    ...
    split = factor(body, "r", lambda name: name == "x", _hoist)
    assert split.varying == ("x", "x")
    assert sorted(split.invariant) == [("a", False), ("b", False)]


def test_a_sum_is_not_a_product() -> None:
    assert factor(Body([_asn("r", "add", "x", "y")]), "r", lambda name: name == "x", ring) is None
...
def test_a_varying_divisor_refuses() -> None:
    ...
    assert factor(Body([_asn("r", "divide", "x", "y")]), "r", lambda name: name in {"x", "y"}, _hoist) is None


def test_a_lone_atom_is_a_one_factor_product() -> None:
    """Structural refusals are the bridge's; usefulness is the rule's. A cone that is one opaque
    weight splits into one varying factor and no invariant — the caller declines that, not this."""
    split = factor(Body([_asn("w", "exp", "d")]), "w", lambda name: name == "w", _hoist)
...
def test_welfords_residual_linearizes_to_its_monomials_in_the_stream() -> None:
    ...
    terms = _monomials(linearize(body, "sq", lambda name: name == "s", ring))
...
def test_a_stream_under_an_uninterpreted_call_refuses() -> None:
    assert linearize(Body([_asn("w", "exp", "s")]), "w", lambda name: name == "s", ring) is None


def test_nothing_streaming_refuses() -> None:
    assert linearize(Body([_asn("r", "multiply", "a", "b")]), "r", lambda name: name == "s", ring) is None
...
def test_the_softmax_carrier_reads_as_a_pivot_and_two_rescaled_channels() -> None:
    ...
    assert linearize(program.body, "m", lambda name: name in {"m", "m__o"}, ring) is None
    for state in ("d", "o"):
        terms = _monomials(linearize(program.body, state, lambda name, s=state: name in {s, f"{s}__o"}, ring))
...
def test_welfords_variance_channel_carries_a_residual_the_other_states_feed() -> None:
    ...
    mean = _monomials(linearize(program.body, "mu", lambda name: name in {"mu", "mu__o"}, ring))
    assert set(mean) == {(("mu", 1),), (("mu__o", 1),)}
    assert all(coeff.free_symbols <= {sympy.Symbol("n"), sympy.Symbol("n__o")} for coeff in mean.values())

    m2 = _monomials(linearize(program.body, "q", lambda name: name in {"q", "q__o"}, ring))
...
def test_an_uninterpreted_op_compares_structurally() -> None:
    ...
    a, b = values("a b")
    assert equal(exp(a - b).expr, exp(a - b).expr)
    assert not equal(exp(a - b).expr, exp(b - a).expr)
    assert not equal(maximum(a, b).expr, maximum(b, a).expr)


def test_ring_identities_decide_equality() -> None:
    x, y = values("x y")
    assert equal(((x + y) * x).expr, (x * x + y * x).expr)
    assert equal((x / y * y).expr, x.expr)
...
def test_an_index_expression_keeps_its_integer_meaning() -> None:
    ...
    assert symbolic(Var("i") / Literal(4, "int")) == sympy.floor(sympy.Symbol("i") / 4)
    assert symbolic(Var("i") % Literal(4, "int")) == sympy.Mod(sympy.Symbol("i"), 4)


def test_every_ring_op_has_a_meaning_and_exp_is_not_in_the_ring() -> None:
    assert all(symbolic(Var("a")) is not None for _ in RING)
