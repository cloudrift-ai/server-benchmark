...
from emmy.compiler.ir.schedule import Placement, Reduce, Tile
...
def test_a_slice_keyed_against_derived_material_prints_in_the_schedule_region() -> None:
    ...
    tile.schedule["TILE@pj"] = Tile(regs=(1, 64))  # a key no stored node claims
    ...
    Sched(plain.op, plain.schedule).put("REDUCE", fold, Reduce.of(reg=4))
...
def test_slices_annotate_a_node_only_when_the_owning_tileop_supplies_them() -> None:
    ...
    Sched(scheduled.op, scheduled.schedule).put("REDUCE", fold, Reduce.of(reg=4))
