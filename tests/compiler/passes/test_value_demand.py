...
def test_affine_coordinate_bound_proves_repetition_without_an_extra_axis() -> None:
    m = Axis("m", Dim(4))
    k = Axis("k", Dim(8))
    cell = Loop(
        axis=k,
    ...
    op = LoopOp(body=Body((Loop(axis=m, body=Body((cell,))),)))
    group = next(group for group in value_demands(op) if group.live_outputs == ("x_f8",))
    assert not group.occurrences[0].repeated_axes
    assert group.repeated
...
def test_alpha_equivalent_fanout_uses_one_value_class() -> None:
