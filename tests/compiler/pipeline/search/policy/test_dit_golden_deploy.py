...
`tile/_schedule.py` used to carry `_RTX4080_DIT_CONTRACTION_DEFAULTS` — a hardcoded
`(a_computed, b_trans, M, N, K) -> (TILE, STAGE, REDUCE)` dict narrowing the enumerated rows on a
deterministic RTX-4080 deploy. Its two PLAIN-A entries now live in `goldens/rtx4080_sm89.yaml`
...
from emmy.compiler.pipeline.search.golden import GOLDEN_CONFIGS, MatmulGoldenConfig, matmul_snippet
...
def _shipped(name: str) -> MatmulGoldenConfig:
...
def _resolve_picks(M: int, N: int, K: int, monkeypatch) -> list[dict]:
    """The rows the GOLDEN tier picked during one full greedy resolve of the matmul.
    ...
    graph, _, _ = graph_from_code(matmul_snippet(M, N, K, "fp16", True))
...
def test_the_shipped_dit_golden_decides_the_live_deploy(name, M, N, K, monkeypatch) -> None:
    ...
    picks = _resolve_picks(M, N, K, monkeypatch)
