"""The AnalyticPrior weights artifact: the repo-checked default's schema, the
``EMMY_ANALYTIC_FILE`` override, and the hard-error load semantics (a missing or
...
from emmy.compiler.pipeline.search.prior.analytic import _DEFAULT_FILE, _PARAM_KEYS, AnalyticPrior


def test_shipped_artifact_is_current_and_complete():
    """The repo-checked default: current featurizer vocabulary, both weight sets,
    all scoring params — the schema gate that keeps `AnalyticPrior()` loadable on
    a fresh clone."""
...
def test_env_override_scores_through_the_file(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("EMMY_ANALYTIC_FILE", str(path))
    p = AnalyticPrior()
...
def test_explicit_kwargs_win_over_the_file(tmp_path, monkeypatch):
    ...
    path.write_text(json.dumps(_artifact()))
    monkeypatch.setenv("EMMY_ANALYTIC_FILE", str(path))
    p = AnalyticPrior(weights={"D_square": 1.0}, scale=1.0)
...
def test_missing_file_is_a_hard_error(tmp_path, monkeypatch):
    monkeypatch.setenv("EMMY_ANALYTIC_FILE", str(tmp_path / "nope.json"))
