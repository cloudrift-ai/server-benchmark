...
def test_native_exl3_moe_matches_decoded_route_and_captured_replay():
    """The pinned fused tier consumes coded K2/cb2 tensors, preserves route metadata, and
    releases every pointer owner after repeated outer-graph replay."""
    pytest.importorskip("cupy")
    import gc
    import weakref
    ...
    if torch.cuda.get_device_properties(torch.cuda.current_device()).major < 8:
        pytest.skip("the native EXL3 MoE tier is qualified on SM80 and newer GPUs")

    from emmy.compiler.backend.gpu_lock import gpu_lock
    from emmy.serving.exl3_moe import Exl3MoeM1, Exl3MoeSpec
    from tests.support.checkpoints import exl3_linear_tensors

    warm = torch.empty(1, 128, dtype=torch.float16, device="cuda")
    F.linear(warm, torch.empty(128, 128, dtype=torch.float16, device="cuda"))
    torch.cuda.synchronize()
    del warm
    torch.cuda.empty_cache()
    allocated_before = torch.cuda.memory_allocated()
    num_experts = 4
    inputs, decoded = {}, {name: [] for name in ("w_gate", "w_up", "w_down")}
    for name in decoded:
        coded, suh, svh = [], [], []
        for expert in range(num_experts):
            tensors, weight = exl3_linear_tensors(f"{name}.{expert}", 128, 128, K=2, cb=2)
            coded.append(tensors[f"{name}.{expert}.trellis"].view(torch.int16))
            suh.append(tensors[f"{name}.{expert}.suh"])
            svh.append(tensors[f"{name}.{expert}.svh"])
            decoded[name].append(torch.from_numpy(weight))
        inputs[name] = torch.stack(coded).cuda()
        inputs[f"{name}_suh"] = torch.stack(suh).cuda()
        inputs[f"{name}_svh"] = torch.stack(svh).cuda()

    runtime = Exl3MoeM1(Exl3MoeSpec(2, 2, 128, 128, num_experts, 2, 128))
    pointers = runtime.pointer_tables(inputs)
    source_refs = [weakref.ref(tensor) for tensor in inputs.values()]
    workspace_refs = [weakref.ref(runtime.output), weakref.ref(runtime._locks)]
    x = (torch.randn(1, 128, device="cuda") * 0.05).half()
    scores = torch.tensor([[1.5, 0.75]], dtype=torch.float16, device="cuda")
    indices = torch.tensor([[3, 1]], dtype=torch.int64, device="cuda")

    with gpu_lock(), cp.cuda.Stream.from_external(torch.cuda.current_stream()):
        eager = runtime(x, scores, indices, pointers).clone()
    assert torch.equal(runtime._expert_count, torch.tensor([0, 1, 0, 1, 0], dtype=torch.int64, device="cuda"))
    assert torch.equal(runtime._token_sorted, torch.zeros(2, dtype=torch.int64, device="cuda"))
    assert torch.equal(runtime._weight_sorted, torch.tensor([0.75, 1.5], dtype=torch.float16, device="cuda"))

    reference = torch.zeros_like(eager)
    for score, expert in zip(scores.flatten(), indices.flatten(), strict=True):
        gate = F.linear(x, decoded["w_gate"][expert].cuda())
        up = F.linear(x, decoded["w_up"][expert].cuda())
        down = F.linear(F.silu(gate) * up, decoded["w_down"][expert].cuda())
        reference += score.float() * down.float()
    torch.testing.assert_close(eager, reference, rtol=3e-2, atol=3e-2)

    graph = torch.cuda.CUDAGraph()
    with gpu_lock(), torch.cuda.graph(graph):
        with cp.cuda.Stream.from_external(torch.cuda.current_stream()):
            captured = runtime(x, scores, indices, pointers)
    graph.replay()
    first = captured.clone()
    graph.replay()
    second = captured.clone()
    torch.cuda.synchronize()
    torch.testing.assert_close(first, eager, rtol=0, atol=1e-5)
    torch.testing.assert_close(second, eager, rtol=0, atol=1e-5)

    scores.copy_(torch.tensor([[0.5, 1.25]], dtype=torch.float16, device="cuda"))
    indices.copy_(torch.tensor([[0, 2]], dtype=torch.int64, device="cuda"))
    graph.replay()
    changed = captured.clone()
    graph.replay()
    changed_replay = captured.clone()
    torch.cuda.synchronize()
    changed_reference = torch.zeros_like(changed)
    for score, expert in zip(scores.flatten(), indices.flatten(), strict=True):
        gate = F.linear(x, decoded["w_gate"][expert].cuda())
        up = F.linear(x, decoded["w_up"][expert].cuda())
        down = F.linear(F.silu(gate) * up, decoded["w_down"][expert].cuda())
        changed_reference += score.float() * down.float()
    assert torch.equal(runtime._expert_count, torch.tensor([1, 0, 1, 0, 0], dtype=torch.int64, device="cuda"))
    assert torch.equal(runtime._weight_sorted, torch.tensor([0.5, 1.25], dtype=torch.float16, device="cuda"))
    torch.testing.assert_close(changed, changed_reference, rtol=3e-2, atol=3e-2)
    torch.testing.assert_close(changed_replay, changed, rtol=0, atol=1e-5)
    assert not torch.equal(changed, eager), "captured replay must consume the live route metadata"

    del inputs
    gc.collect()
    assert all(ref() is not None for ref in source_refs), "pointer tables must retain their coded tensor owners"
    del (
        graph,
        captured,
        first,
    ...
    torch.cuda.empty_cache()
    assert all(ref() is None for ref in source_refs + workspace_refs)
    assert torch.cuda.memory_allocated() <= allocated_before + 1024 * 1024
