...
from emmy.compiler.loader.quant import mxfp4_sm70_mma884_storage
...
def test_grouped_mxfp4_stage_accepts_retained_sm70_storage_without_second_weight_buffer():
    graph = trace_grouped_mxfp4_stage(
        groups=4,
        rows_per_group=16,
        experts=3,
        out_features=32,
        in_features=32,
        storage=mxfp4_sm70_mma884_storage((3, 32, 32)),
    )
    try:
        set_target((7, 0))
        with pinned_knobs({"TILE": "mma_m8n8k4_f16_f32/f1x1", "WORK": "w1x1", "STAGE": "d1/smem"}):
    ...
    assert plan.inputs == ["grouped_x", "weight", "expert_ids", "weight_scale"]
    buffers = {buffer.name: buffer for buffer in plan.buffers}
    assert buffers["weight"].dtype.name == "i32"
    assert tuple(dim.as_static() for dim in buffers["weight"].shape) == (3, 32, 4)
    assert tuple(dim.as_static() for dim in buffers["weight_scale"].shape) == (3, 1, 32)
    assert len(plan.launches) == 1
    assert tuple(buffer.name for buffer in plan.buffers if buffer.role == "scratch") == ()
